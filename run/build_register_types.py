...
class GodotExtension:
    ...
    @property
    def name(self):
        return next(iter(self.name.split(".")), "")
...
def get_source_files(path_string="src/"):
    ...
    for filename in os.listdir(path):
        if os.path.isdir(path / filename):
            results.extend(get_source_files(path / filename))
        else:
            ...
            if is_cpp_header or is_c_header and not is_register_types:
...
def main():
    ...
    for godot_extension in get_source_files():
        include_string += f"#include <{godot_extension.path}>" + "\n"
